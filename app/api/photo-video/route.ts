    ...
    const scenes = (connectPhotos ? [inputs[0]] : inputs).map((input, index) => {
      ...
      return {
        ...
        shots: [{
          ...
          prompt,
          motionProfile: {
            bodyMotion: connectPhotos ? "large" as const : "small" as const,
            characterCount: 2,
            hasPhysicalContact: true,
            hasObjectTransfer: false,
            hasWalkingOrRunning: connectPhotos,
            hasLargePoseChange: connectPhotos,
            changesLocation: connectPhotos,
            requiresEndFrame: connectPhotos,
          },
        }],
        input,
