...
worker 자체는 최대 9개의 정렬된 키프레임을 균등한 프레임 간격으로 받을 수 있어,
카메라 위치 차이가 큰 장면에는 별도의 브리지 프레임을 추가할 수 있다.
다중 브리지 모드에서는 시작과 끝만 강도 1.0으로 고정하고 중간 프레임은 0.45로
조건화해, 모델이 각 정지화면으로 급격히 스냅하지 않고 이동 동작을 생성할 여지를 둔다.
